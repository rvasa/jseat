...
public class Project
{
	...
	public Project(String inputFileName)
	{
		...
		projectName = inputFileName.substring(
				...
				(inputFileName.length() - JSeatFileType.PROJECT.toString()
						.length()));
	}
	...
	public Project(String inputFileName, String outputFileName, int numThreads)
	{
		...
		projectName = outputFileName.substring(
				...
				(outputFileName.length() - JSeatFileType.PROJECT.toString()
						.length()));
	}
	...
	private void loadProjectFromFile() throws IOException
	{
		...
		while (br.ready())
		{
			...
			else if (line.startsWith("$") && (line.indexOf(",") != -1))
			{
				...
				this.projectName = toks[0].substring(line.indexOf("$")+1, toks[0]
						.length());
			...
			else if (line.startsWith("$"))
			{
				...
				versions = Integer.parseInt(line.substring(1, line.length()));
			} // Version
			else
	...
	private void writeNewProjectFile()
	{
		...
		try
		{
			...
			bw
					.write("# Java Software Evolution Analysis Tool (JSeat) Project File #");
			bw.newLine();
			bw.write("# Created: ");
			...
			// Close stream.
			bw.close();
			logger.log(Level.ALL, "Finished writing project file to: "
					+ filename);
	...
	private void writeVersionFiles(BufferedWriter bw, SerializeType type)
			throws IOException
	{
		bw.write("$");
		...
		{
			String file = String.valueOf(i) + ", " + hmd.getNameOf(i) + ", "
					+ hmd.getPathOf(i);
			bw.write(file);
			bw.newLine();
		}
		...
		for (int i = 0; i < args.length; i++)
		{
			...
			if (args[i].equals("-i"))
			{
				if (args.length >= i + 2)
				{
					...
					System.err
							.println("You must specify a filename after the -i command.");
			...
			} else if (args[i].equals("-t"))
			{
				if (args.length >= i + 2)
				{
					...
					System.err
							.println("You must specify a number after the -t command.");
			}
		}
...
//		project.addObserver(project);
		...
		try
		{
			MetricDataConverter fromConverter = new CSVConverter(
					SerializeType.CLASSES);
			VersionMetricData vmd = fromConverter.deSerialize(new FileReader(
					"D:\\MyASMProject\\data\\1.cme"));
			MetricDataConverter fromConverter2 = new CSVConverter(
					SerializeType.DEPENDENCIES);
			VersionMetricData vmd2 = fromConverter2.deSerialize(new FileReader(
					"D:\\MyASMProject\\data\\1.dep"));
